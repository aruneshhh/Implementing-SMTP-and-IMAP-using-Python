        ...
        if os.path.isfile(filepath) == False: 
            ...
            while True:
                ...
                if os.path.isfile(alt_filepath) == False:
                    with open(alt_filepath, 'wb') as f:f.write(part.get_payload(decode = True))
                    break
                a += 1
...
####imap.store(search[0].split()[::-1][listbox.curselection()[0]], '+FLAGS', '"[Gmail]/Trash"')####
    if Gmail_Login == True:
        ...
        try:
            ...
            else:messagebox.askokcancel('Wait!!', 'No mail has been selected!!')
        except:
...
def recv_mail(mailbox = 'INBOX'):
    ...
    scrl.delete(1.0, END)
    listbox.delete(0)
    if attach_label != None: attach_label.config(text = "")
    imap.select(mailbox)
    global mb
    mb = mailbox
...
root_password_entry = Entry(root_frame, font = ('Verdana', 16), show = '*', textvariable = 
root_password_variable, width = 30)
